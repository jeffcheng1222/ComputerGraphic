...
void Shader::setUnif(const std::string & name, unsigned value) const
{
	glUniform1ui(glGetUniformLocation(id, name.c_str()), (unsigned)value);
}
...
void Shader::setUnif(const std::string& name, Eigen::Affine3f trans) const
{
	glUniformMatrix4fv(glGetUniformLocation(id, name.c_str()), 1, GL_FALSE, trans.data());
}
...
Shader& Shader::now() {
	if (current == nullptr)
		...
		throw std::runtime_error("Not using any shader now!");
	return *current;
}